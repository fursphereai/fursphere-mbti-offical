...
export const getDownloadImageUrl5 = async (surveyData: SurveyData, mbti: string, isFromUserProfile: boolean) => {
  const elementToCapture = document.getElementById('download-5');
  if (!elementToCapture) {
...
export const handleDownload5 = async (surveyData: SurveyData, mbti: string, isFromUserProfile: boolean) => {
  const elementToCapture = document.getElementById('download-5');
  if (!elementToCapture) {
    console.error('Element not found');
    return;
  ...
  try {
    ...
    if (isMobile) {
      ...
      if (navigator.share) {
        ...
        const file = new File([blob], `${surveyData.pet_info.PetName}-page5.jpeg`, { type: 'image/jpeg' });
    ...
    link.download = `${surveyData.pet_info.PetName}-page5.jpeg`;
    link.href = dataUrl;
    link.click();
  } catch (error) {
    console.error('dom-to-image error:', error);
...
export default function BreakdownI({ aiResult, surveyData, isFromUserProfile }: { aiResult: string, surveyData: SurveyData, isFromUserProfile: boolean }) {
  ...
  return (
      <motion.div 
        ...
        <div className="absolute top-[112px]  w-full px-[40px]">
            ...
            {result.ai_output.text.i_label.charAt(0)}
            </h2>
        </div> 
