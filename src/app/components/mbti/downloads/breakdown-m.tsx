...
export const getDownloadImageUrl2 = async (surveyData: SurveyData, mbti: string, isFromUserProfile: boolean) => {
...
//         const file = new File([blob], `${surveyData.pet_info.PetName}-page2.jpeg`, { type: 'image/jpeg' });
...
//     link.download = `${surveyData.pet_info.PetName}-page2.jpeg`;
//     link.href = dataUrl;
//     link.click();
//   } catch (error) {
//     console.error('dom-to-image error:', error);
  ...
  return (
      <motion.div 
        ...
        <div className="absolute top-[624px]  w-full px-[40px]">
            ...
            [ {result.ai_output.text.m_label}: {Math.round(Math.abs(result.ai_output.text.m_score) / 2 + 50)}% ]
            </h2>
        </div> 
        ...
        <div className="absolute  w-[300px] top-[674px] left-[40px] h-[20px] rounded-[32px] "
          style={{ 
          width: `${Math.round(Math.abs(result.ai_output.text.m_score) / 2 + 50) * 3}px`,
          backgroundColor: 
