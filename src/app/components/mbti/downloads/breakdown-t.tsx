...
export const getDownloadImageUrl4 = async (surveyData: SurveyData, mbti: string, isFromUserProfile: boolean) => {
  const elementToCapture = document.getElementById('download-4');
  if (!elementToCapture) {
...
// export const handleDownload4 = async (surveyData: SurveyData, mbti: string, isFromUserProfile: boolean) => {
//   const elementToCapture = document.getElementById('download-4');
//   if (!elementToCapture) {
//     console.error('Element not found');
//     return;
...
//         const file = new File([blob], `${surveyData.pet_info.PetName}-page4.jpeg`, { type: 'image/jpeg' });
...
//     link.download = `${surveyData.pet_info.PetName}-page4.jpeg`;
//     link.href = dataUrl;
//     link.click();
//   } catch (error) {
//     console.error('dom-to-image error:', error);
...
export default function BreakdownT({ aiResult, surveyData, isFromUserProfile }: { aiResult: string, surveyData: SurveyData, isFromUserProfile: boolean }) {
  ...
  return (
      <motion.div 
        ...
        <div className="absolute top-[112px]  w-full px-[40px]">
            ...
            {result.ai_output.text.t_label.charAt(0)}
            </h2>
        </div> 
        ...
        <div className="absolute top-[624px]  w-full px-[40px]">
            ...
            [ {result.ai_output.text.t_label}: {Math.round(Math.abs(result.ai_output.text.t_score) / 2 + 50)}% ]
            </h2>
        </div> 
        ...
        <div className="absolute  w-[300px] top-[674px] left-[40px] h-[20px] rounded-[32px] "
          style={{ 
          width: `${Math.round(Math.abs(result.ai_output.text.t_score) / 2 + 50) * 3}px`,
          backgroundColor: 
